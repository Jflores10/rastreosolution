...
class DespachoController extends Controller
{
    ...
    public function end(Request $request, $id)
    {
        ...
        date_add($ini, date_interval_create_from_date_string($defaultMinutesIni . ' minutes'));
        date_add($fin, date_interval_create_from_date_string($defaultMinutesFin . ' minutes'));
        date_add($ini_corta, date_interval_create_from_date_string('10 hours'));
        date_add($fin_corte, date_interval_create_from_date_string('10 hours'));

        $fini = new UTCDateTime(($ini->getTimestamp() * 1000));
        $ffin = new UTCDateTime(($fin->getTimestamp() * 1000));
        ...
        if (sizeof($recorridos) > 0) {
            foreach ($despacho->puntos_control as $punto_control) {
                ...
                if (isset($puntoControlObj)) {
                    ...
                    if ($primerpunto) {
                        ...
                        $puntoControlEsperado_inicio = Recorrido::orderBy('fecha_gps', 'desc')
                            ->where('tipo', 'GTGEO')
                            ->where('fecha_gps', '>=', $fini)
                        ...
                        $puntoControlEsperado_salida = Recorrido::orderBy('fecha_gps')
                            ->where('tipo', 'GTGEO')
                            ->where('fecha_gps', '<=', $ffin)
                        ...
                        $puntoControlEsperado = $puntoControlEsperado_inicio ?? $puntoControlEsperado_salida;
                    } else {
                        /*************** SIGUIENTES PUNTOS -> SEGÚN CALCULO ****************/
                ...
                if (isset($puntoControlEsperado)) {
                    ...
                    $fini = new UTCDateTime(($fechaGPS->getTimestamp() * 1000));
            ...
            if (count($despacho->puntos_control) > 0) {
                $contador = ($despacho->puntos_control[count($despacho->puntos_control) - 1]['contador_marca'] ?? 0);
            ...
            $despacho->estado = 'C';
            $despacho->auxiliar = [];
            $despacho->corte_tubo = 'No';
            $despacho->latitud_corte = 0.0;
            $despacho->longitud_corte = 0.0;
